...
// ValidarToken confirma se o token é valido
...
// ExtrairUsuarioID retorna o usuarioId salvo no token
...
func retornarChaveVerificacao(token *jwt.Token) (interface{}, error) {
	if _, ok := token.Method.(*jwt.SigningMethodHMAC); !ok {
		return nil, fmt.Errorf("Método de assinatura não compativel. %v", token.Header["alg"])
	}
